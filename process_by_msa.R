...
state_codes <- read_csv("data/sas_state_code.csv") |>
    rename(STATE = CODE)

ccaei103 <- open_dataset("parquet/ccaei103") |>
    select(ENROLID, MSA, schizophrenia, AGE) |>
...
population <- read_csv("data/nst-est2019-alldata.csv") |>
    filter(SUMLEV == "040") |>
...
rates <- inner_join(population, ccaei103) |>
    ...
    write_csv("results/schizophrenia_incidence_rates_population.csv")
